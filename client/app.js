(function (window) {
  ...
  const guess = () => {
    ...
    if (lastGuess < min || lastGuess > max || isNaN(lastGuess)) {
      document.getElementById("error").style.display = "block";
      return;
    }
console.log(lastGuess)
    apiGETRequest("/guess", guessHandler, "guess_number=" + lastGuess);
  ...
  const guessHandler = (res) => {
    ...
    // in range but wrong number
      ...
      displayHTML("hint", `HINT: It is ${moreOrLess} than ${lastGuess}`, true);
      displayHTML("attempts", `${attemptsLeft} out of ${attempts}`, true);
      displayHTML("currentPlayer", currentPlayer, true);
  }
